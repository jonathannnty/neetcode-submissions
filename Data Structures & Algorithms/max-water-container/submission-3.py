class Solution:
    def maxArea(self, heights: List[int]) -> int:
        ...
        while left < right:
            ...
            print(f"left: {left} left element: {heights[left]}")
            print(f"right: {right} right element: {heights[right]}")
            print(f"possible max: {(right - left) * min(heights[left], heights[right])}")
